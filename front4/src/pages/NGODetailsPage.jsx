...
import { ngoAPI, contactAPI } from '../services/api'
...
export default function NGODetailsPage(){
    ...
    useEffect(() => {
        if (user && ngo) {
            // Проверяем, есть ли НКО в избранном пользователя
            const favorites = user.favorites || []
            setIsFavorite(favorites.includes(ngo.id))
        }
    }, [user, ngo])
    ...
    const loadNGO = async () => {
        setLoading(true)
        try {
            const data = await ngoAPI.getById(id)
            setNgo(data)
        } catch (error) {
            console.error('Failed to load NGO:', error)
            showToast('Ошибка загрузки организации', 'error')
        } finally {
            setLoading(false)
        }
    }
    ...
    const toggleFavorite = async () => {
        ...
        try {
            if (isFavorite) {
                await ngoAPI.removeFromFavorites(id)
                setIsFavorite(false)
                showToast('Удалено из избранного', 'success')
            } else {
                await ngoAPI.addToFavorites(id)
                setIsFavorite(true)
    ...
    // Соцсети из данных НКО (может быть в разных полях API)
    const socialLinks = {
        website: ngo.website || null,
        vk: ngo.vk || ngo.social_links?.vk || null,
        telegram: ngo.telegram || ngo.social_links?.telegram || null
    }
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-8">
                ...
                <h1 className="text-4xl font-bold mb-4" style={{ color: '#1a2165' }}>{ngo.name}</h1>
                <p className="text-lg" style={{ color: '#3a3a39' }}>{ngo.city}</p>
            </div>
            ...
            {/* Описание */}
            <div className="mb-8">
                <h2 className="text-2xl font-bold mb-4" style={{ color: '#1a2165' }}>О организации</h2>
                <div className="prose max-w-none">
                    <p className="text-base leading-relaxed mb-4 whitespace-pre-line" style={{ color: '#3a3a39' }}>
                        {ngo.description || ngo.full_description}
                    </p>
                </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-8">
                <div className="card">
                    <h3 className="text-xl font-bold mb-4" style={{ color: '#1a2165' }}>Проекты</h3>
                ...
                <div className="card">
                    <h3 className="text-xl font-bold mb-4" style={{ color: '#1a2165' }}>Цели</h3>
            ...
            <div className="card mb-8">
                <h3 className="text-xl font-bold mb-4" style={{ color: '#1a2165' }}>Как принять участие</h3>
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-8">
                <div className="card">
                    <h3 className="text-xl font-bold mb-4" style={{ color: '#1a2165' }}>Контактная информация</h3>
                ...
                <div className="card">
                    <h3 className="text-xl font-bold mb-4" style={{ color: '#1a2165' }}>Связаться с нами</h3>
            ...
            <div className="flex flex-wrap gap-3">
                <Button
                    variant={isFavorite ? 'outline' : 'outline-accent'}
