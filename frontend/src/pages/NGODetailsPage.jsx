...
import { ngoAPI, contactAPI } from '../utils/api'
...
export default function NGODetailsPage(){
    ...
    if (!ngo) {
        return (
            <div className="text-center py-12">
                <p className="text-gray-500 text-lg">НКО не найдено</p>
            </div>
        )
    }
    ...
    const toggleFavorite = async () => {
        ...
        try {
            const isFavorite = ngo.is_favorite
            await ngoAPI.toggleFavorite(ngo.id, isFavorite)
            
            // Обновляем локальное состояние
            setNgo({ ...ngo, is_favorite: !isFavorite })
            
            // Обновляем пользователя
            if (user) {
                const updatedFavorites = isFavorite
                    ? (user.favorites || []).filter(favId => favId !== ngo.id)
                    : [...(user.favorites || []), ngo.id]
    ...
    // Пример данных для соцсетей (в реальном приложении из API)
    const socialLinks = {
        vk: ngo.website?.includes('vk.com') ? ngo.website : null,
        telegram: ngo.website?.includes('t.me') ? ngo.website : null,
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            {ngo.logo && (
                <div className="mb-8">
                    <div className="w-full h-64 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
                        <img 
                            src={ngo.logo.startsWith('http') ? ngo.logo : `http://localhost:8000${ngo.logo}`} 
                            alt={ngo.name} 
            ...
            <div className="mb-8">
                ...
                <div className="prose max-w-none">
                    <p className="text-[#454545] text-base leading-relaxed mb-4 whitespace-pre-line">
                        {ngo.description || ngo.short_description}
                    </p>
                </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
                <Button
                    variant={ngo.is_favorite ? 'accent' : 'outline'}
                    ...
                    ariaLabel={ngo.is_favorite ? 'Удалить из избранного' : 'Добавить в избранное'}
                >
                    {ngo.is_favorite ? 'В избранном' : 'В избранное'}
                </Button>
                {ngo.website && (
                    <Button
                        variant="primary"
