...
export default function NGOListPage(){
    ...
    useEffect(() => {
        const loadCategories = async () => {
            try {
                ...
                const categoryList = ['Все', ...data.map(c => c.name)]
        ...
        const loadNGOs = async () => {
            ...
            try {
                const params = {
                    page: currentPage,
                    ...(city && { city }),
                    ...(category !== 'Все' && { category: category.toLowerCase().replace(/\s+/g, '-') }),
                    ...(searchQuery && { search: searchQuery })
                }
                const data = await api.get('/ngos/', params)
                // DRF возвращает результаты в поле results при пагинации
    ...
    const filteredList = useMemo(() => {
        return ngos.filter(n => {
            // Если есть поисковый запрос, фильтруем локально (API уже делает поиск, но для точности)
            if (searchQuery) {
    ...
    const totalPages = Math.ceil((ngos.length || 0) / itemsPerPage)
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="text-center py-8">
                <h1 className="text-5xl font-extrabold text-primary mb-4">Организации</h1>
                <p className="text-xl text-gray-600">Найдите инициативы в вашем городе и присоединяйтесь</p>
            </div>
