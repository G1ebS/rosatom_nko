...
export default function MaterialDetailsPage() {
    ...
    useEffect(() => {
        const loadMaterial = async () => {
            ...
            try {
                ...
                // Проверяем, сохранен ли материал
                if (user) {
                    try {
                        const library = await api.get('/library/')
                        const saved = library.some(item => item.material?.id === data.id || item.material === data.id)
                        setIsSaved(saved)
                    } catch (error) {
                        console.error('Failed to check if material is saved:', error)
                    }
                }
            } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (isSaved) {
                await api.delete(`/materials/${id}/unsave/`)
            ...
            console.error('Failed to save/unsave material:', error)
    ...
    const materialType = material.type || material.course || 'Другое'
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            {/* Контент материала */}
            <div className="card">
                {(materialType === 'Видео' || materialType === 'видео') ? (
                ...
                ) : (materialType === 'PDF' || materialType === 'pdf') ? (
                    <div className="space-y-6">
